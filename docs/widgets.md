...
Инструкции по интеграции: [Подключение виджета на страницу "Карточка товара" Корзина" и "Спасибо"](#Podklyuchenie-popup-vidzheta-na-stranitsy-Kartochka-tovara-Korzina-i-Spasibo)
<br/><br/>

  </details>

<a class="anchor" name="iframe-profits"></a>
...
Инструкции по интеграции: [Установка iframe формы на отдельной странице "Установка и настройка"](#Ustanovka-iframe-formy-na-otdelnoi-stranitse-Ustanovka-i-nastroika)

  </details>

<br/>

<a class="anchor" name="Podklyuchenie-popup-vidzheta-na-stranitsy-Kartochka-tovara-Korzina-i-Spasibo"></a>
...
**data-city** [обязательно] - город отображения виджета; [список доступных городов](#Goroda) (с учетом регистра)

<a class="anchor" name="data-page"></a>

**data-page** [обязательно] - принимает строку, определяющую на какой странице запускается скрипт. В данном случае ожидается значение _product_ (_product_ страница товара или списка товаров; _cart_: страница корзины магазина).
...
<a class="anchor" name="advanced-script-installation"></a>
...
<a class="anchor" name="service-data"></a>Корзина представляет из себя массив следующих объектов:
...
<a class="anchor" name="Bloki-uslug"></a>

### Блоки услуг

Рядом с товаром, в том месте, где вы хотите отобразить "галочку" с опцией установки, вставьте следующий код. "Галочка" будет отображаться только на тех товарах, для которых доступна услуга установки. Скрытием и отображением "галочки" управляет скрипт Clicker, руководствуясь [правилами сопоставления товар→услуга](#Pravila-sopostavleniya-tovar-usluga), поэтому вам не нужно ставить дополнительные условия в коде, вы можете добавить элемент на все товары.
...
<a class="anchor" name="data-match-properties"></a>

**data-match-properties** [обязательный] - JSON массив объектов, по которым будет проводиться сопоставление товаров и услуг. Каждый объект содержит два параметра, key и value, и описывает одно свойство товара. Свойства товара могут быть любыми, это настраивается индивидуально под каждый магазин (наиболее часто используемые - категория товаров), но бывают и различные особенности (например, размер телевизора, мощность кондиционера, наличие гидромассажа у ванны и пр.). Логика сопоставления описана в [Правилах сопоставления товар → услуга](#Pravila-sopostavleniya-tovar-usluga). Пример массива:
...
<a class="anchor" name="checkbox-block-additional"></a>
...
<a class="anchor" name="installation-service-block_price-value"></a>

**[data-is-sb-price-value].installation-service-block_price-value** - элемент, в который проставляется значение цены
...
Подключение скрипта на страницу "Корзина" происходит аналогичным образом, с несколькими блоками [installation-service-block](#Bloki-uslug), для каждого товара в корзине. За исключением лишь того, что атрибут _[data-page](#data-page)_ должен быть равным _cart._
...
<a class="anchor"name="Kratkii-variant"></a>
...
<a class="anchor"name="Podrobnyi-variant"></a>
...
<a class="anchor"name="data-always-visible"></a>
...
<a class="anchor" name="cart-data"></a>

Содержимое одного объекта в массиве cart практически идентично тому, что вы использовали при встраивании блока услуги на страницу, а именно:
...
<a class="anchor"name="get-order-data"></a>
...
<a class="anchor"name="Ustanovka-iframe-formy-na-otdelnoi-stranitse-Ustanovka-i-nastroika"></a>
...
**data-city -** город отображения виджета, [список доступных городов](#Goroda); если не передан, будет использован город по умолчанию для магазина  
...
**Данные одной услуги**
Для одной услуги достаточно передать данные товара в отдельные поля ([правила сопоставления](#Pravila-sopostavleniya-tovar-usluga) описаны ниже)
...
<a class="anchor"name="Pravila-sopostavleniya-tovar-usluga"></a>
...
<a class="anchor"name="Dostupnye-tsvetovye-temy-vidzhetov"></a>
